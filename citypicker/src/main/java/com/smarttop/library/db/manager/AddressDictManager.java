...
public class AddressDictManager {
    ...
    private AddressBeanNew addressBeanNew;
    ...
    public List<Province> getProvinceBeanList() {
    ...
    public Cun getCunBean(String streetCode) {
        Cursor cursor = db.rawQuery("select * from " + TableField.TABLE_ADDRESS_DICT + " where " + ADDRESS_DICT_FIELD_CODE + "=?", new String[]{streetCode});
        if (cursor != null && cursor.moveToFirst()) {
            Cun cun = new Cun();
            cun.id = cursor.getInt(cursor.getColumnIndex(ADDRESS_DICT_FIELD_ID));
            cun.code = cursor.getString(cursor.getColumnIndex(ADDRESS_DICT_FIELD_CODE));
            cun.name = cursor.getString(cursor.getColumnIndex(TableField.ADDRESS_DICT_FIELD_NAME));
            cursor.close();
            return cun;
        } else {
            return null;
